...
@section('title', 'Yeni Mükellef')

@section('content')
    <div x-data="{ iletisimKanali: '{{ old('iletisim_kanali', '') }}' }">
        <form method="POST" action="{{ route('mukellefler.store') }}">
            @csrf
                    ...
                    <div class="mb-6">
                        ...
                        <select id="tur"
                            ...
                            @foreach($mukellefTurleri as $tur)
                                <option value="{{ $tur->value }}" {{ old('tur') === $tur->value ? 'selected' : '' }}>
                                    {{ ucfirst($tur->value) }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    @foreach($beyannameTipleri as $tip)
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox"
                                   ...
                                   value="{{ $tip->value }}"
                                   {{ in_array($tip->value, old('beyanname_tipleri', [])) ? 'checked' : '' }}
                                   class="rounded border-zinc-300 text-emerald-600 focus:ring-emerald-500">
